...
export default function CreatePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const contract = new ethers.Contract(siteConfig.contractAddress, StampXAbi, signer);

      // Create a unique hash for the business
      const businessHash = ethers.keccak256(
        ethers.solidityPacked(
          ['address', 'string', 'uint256'],
          [address, formData.name, Date.now()]
        )
      );

      // Convert values to Wei
      const threshold = ethers.parseEther(formData.rewardThreshold);
      const amount = ethers.parseEther(formData.rewardAmount);
      ...
      setError(err.message || 'An error occurred while registering the business');
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-gray-50 px-4 py-12">
        <div className="max-w-md mx-auto text-center">
          <p className="text-lg text-gray-600">Please connect your wallet to create a business</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-gray-600">Register your business to start offering loyalty rewards</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-md">
          <div className="space-y-6">
            ...
            {success && (
              <div className="text-green-600 text-sm">Business registered successfully!</div>
            )}
